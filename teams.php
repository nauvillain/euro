...
echo "</div>\n";
if($team_id){
//	$spl=mysqli_query($link,"SELECT * FROM players WHERE team_id='$team_id' ORDER by id");
//	$spl=$DB::qry("SELECT * FROM players WHERE team_id='$team_id' ORDER by id");
	$spl=mysqli_query($link,"SELECT * FROM players WHERE team_id='$team_id' ORDER by id");
	$num_pl=mysqli_num_rows($spl);
	$query_team="select team_id,team_name,players_list from teams where team_id='$team_id'";
	$res_team=mysqli_query($link,$query_team);
	if(mysqli_num_rows($res_team)){
		$team_name=get_team_name(mysqli_result($res_team,0,'team_id'));
		$players_list=mysqli_result($res_team,0,'players_list');
	}	
	echo "<br/>";
...
//	echo "<div id='team_banner' style='background:url(img/".$log."_b.jpg) no-repeat;'>".$team_name."</div>";
	echo "<div class=players_list><table>\n<tr><th colspan='2'>".get_word_by_id(213)."</th></tr>\n";
	...
	if($players_list){
		...
		for($i=0;$i<$num_pl;$i++){
			if($i % 2==0){
				echo "<tr>\n";
			//<td>".($i+1)." -</td>
				echo "<td align='left'> ".mysqli_result($spl,$i,'name')."</td>";
			}
			else
			{
				 echo "<td align='left'> ".mysqli_result($spl,$i,'name')."</td>";

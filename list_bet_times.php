...
if(is_admin($login_id)){
	...
	$res=mysqli_query($link,"SELECT * FROM bets WHERE DATE_SUB(NOW(),INTERVAL 12000 MINUTE)< time_entered ORDER BY time_entered desc") or die(mysqli_error($link));	
	$num=mysqli_num_rows($res);
	echo $num."<br/><br/><br/><br/>";
	echo "<table align='center'>\n";
	for($i=0;$i<$num;$i++){
		$time=mysqli_result($res,$i,'time_entered');
		$player_id=mysqli_result($res,$i,'player_id');
		$id=mysqli_result($res,$i,'bet_id');
		$match_id=mysqli_result($res,$i,'match_id');

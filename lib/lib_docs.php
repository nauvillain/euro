...
function connect_to_documents_database()
{
...
function disp_col($col,$label) {
	echo "<td><font color='green'><b><a href='".$_SERVER['PHP_SELF']."?order=".$col."'>".$label."</a></b></font></td>\n";
}
...
function filesize_format($bytes, $format = '', $force = '')
    {
        ...
        $units = array('TB' => 1099511627776,
                       'GB' => 1073741824,
                       'MB' => 1048576,
                       'KB' => 1024,
                       'B'  => 0);
        
        if (array_key_exists($force, $units)) {
            $qty = $units[$force];
            return sprintf($format, $qty ? $bytes / $qty : $bytes, $force);
        }
        foreach ($units as $unit => $qty) {
            if ($bytes >= $qty)
                return sprintf($format, $qty ? $bytes / $qty : $bytes, $unit);
        }
    }


function make_file_name($filename,$version) {
	$pos=strrpos($filename,".");
	
	return substr($filename,0,$pos)."_".$version.substr($filename,$pos);
}

function get_file_name($stored_file) {
	$index=strrpos($stored_file,"_");
	return substr($stored_file,0,$index);
}
function get_doc($stored_file,$version) {
	$doc_name=get_file_name($stored_file);
	return $doc_name."_".$version;
}

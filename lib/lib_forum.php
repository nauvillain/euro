...
//	if(!$par_id) $new_items--;
	...
	echo ($root?"Re:":"<span class='forum_thread_title'>".utf8_encode($title)."</span>")." (".mysql_result($m_sql,0,'user_name').")";
	//($new_items?", $new_items new":"").
	echo " -- <div class='post_date'>".($par_id?"":"latest ").$str_post_date."</div>";
	echo "</div>\n";
	//echo "<li>\n";
...
function display_options($t_id,$id){
	...
	echo "<a  class='ajax_link' onClick=\"javascript:addPost('".$div_id."')\" >Reply";
	echo "</a>\n";
	echo "</div>\n";
	echo "</div>\n";
}
...
function format_date($d){
	...
	if($time<3600) {
		$f=round($time/60);
		$u=$f." minute ago";
		if($f>1) $u=$f." minutes ago"; 
	}
	if(($time<86400)&&($time>=3600)) {
		$f=round($time/3600);
		$u=$f."hour ago";
		if($f>1) $u=$f." hours ago"; 
	}

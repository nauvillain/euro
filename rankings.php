...
$group=getIfSet($_GET['group']);
$pot=getIfSet($_GET['pot']);
...
$member_query=mysqli_query($link,"SELECT id FROM usergroups WHERE user_id='$login_id'");
if(mysqli_num_rows($member_query)) $group_flag=1; 
else $group_flag=0;
...
for ($i=0;$i<$num;$i++){
	...
	$eur=mysqli_result($res,$i,"bet_money");
	$curr=mysqli_result($res,$i,"current_correct");
	$final_winner=mysqli_result($res,$i,"winner");

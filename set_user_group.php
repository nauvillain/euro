...
global $login_id,$link;
	...
	$res=mysqli_query($link,"SELECT id FROM users where player='1' order by first_name");
	...
	for($i=0;$i<$count;$i++){
		...
		$res2=mysqli_query($link,"SELECT * FROM usergroups WHERE user_id='$login_id' AND member='$id'") or die(mysql_error());
		//if the person is already in the user group, mark as checked
		echo utf8_encode(get_player_full_name($id))."<input type='checkbox' name='v$id' ".(mysqli_num_rows($res2)||($id==$login_id)?"checked":"")." style='float:right;'>";
		echo "</td>";
		if($k==$columns) {
			echo "</tr><tr>\n";
			$k=0;
